	...
	const FString InJsonString, const int32 DesiredSecondsUntilExpiration)
{
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonObject) && JsonObject.IsValid())
	{
		...
		if (JsonObject->TryGetStringField(TEXT("client_email"), TokenInfo.ServiceAccountEmail) &&
			JsonObject->TryGetStringField(TEXT("private_key"), TokenInfo.PrivateKey) &&
			JsonObject->TryGetStringField(TEXT("token_uri"), TokenInfo.TokenUri))
		{
	...
	FRDTGetStringDelegate CallOnComplete, const FRuntimeDataTableOperationParams OperationParams)
{
	...
	if (!CallOnComplete.ExecuteIfBound(CallbackInfo))
	{
		FRuntimeDataTableModule::Print(FString::Printf(TEXT("%hs: CallOnComplete delegate failed."), __FUNCTION__));
	}
}
	...
	FArrayProperty* ArrayProperty, void* ArrayPtr, UObject* OwningObject, FEasyCsvInfo CsvInfo, bool bNameMatch)
{
	...
	if (InnerProperty->IsA(FObjectProperty::StaticClass()))
	{
		...
		{
			if (!CsvInfo.CSV_Keys.IsValidIndex(i) || !CsvInfo.CSV_Map.Find(CsvInfo.CSV_Keys[i]))
			{
				continue;
			}
	...
	TArray<FString> RowKeys, UObject* OwningObject, FString MembersToInclude, const bool bSortColumnsAlphanumerically)
{
	...
	for (int32 i = 0; i < ArrayHelper.Num(); i++)
	{
		...
		// Turn allowlist string into allowlist array
		TArray<FString> MemberAllowlist;
		MembersToInclude.ParseIntoArray(MemberAllowlist, TEXT(","));
		...
		IterateThroughStructPropertyAndMakeRowString(
			...
			bIsUObject, bSortColumnsAlphanumerically, NumValues, TopRow, Row, MemberAllowlist
		);
...
void URuntimeDataTableObject::IterateThroughStructPropertyAndMakeRowString(
		...
		uint8& OutNumValues, FString& OutTopRow, FString& OutRowString, const TArray<FString>& InMemberAllowlist)
{
	...
	const bool bIncludeAllMembers = InMemberAllowlist.Num() == 0;
	...
	for (TFieldIterator<FProperty> It(Struct); It; ++It)
	{
		if (FProperty* Property = *It)
		{
			...
			if (bIncludeAllMembers || InMemberAllowlist.ContainsByPredicate(
...
FString URuntimeDataTableObject::GetAllObjectPropertyNames(const UObject* Object,
	const bool bIncludeOnlyConfigVariables, const bool bIncludeInheritedVariables, FString MemberDenylist)
{
	FString ReturnValue;
	TArray<FString> VarDenylist;

	if (MemberDenylist != "")
	{
		MemberDenylist.ParseIntoArray(VarDenylist, TEXT(","));
	}
	...
	for (TFieldIterator<FProperty> It(Object->GetClass(), IncludeFlags); It; ++It)
	{
		...
		if (VarDenylist.Num() > 0 &&
			VarDenylist.ContainsByPredicate(
	...
	FString InSpreadsheetId, const int32 InSheetId, FString InCsv, FCsvToGoogleSheetsHandler SheetTabData)
{
	...
	if (FJsonSerializer::Deserialize(JsonReader, Spreadsheet) && Spreadsheet.IsValid())
	{
		...
		Spreadsheet->TryGetArrayField(TEXT("sheets"), OutSheets);
		...
		if (OutSheets)
		{
			for (const TSharedPtr<FJsonValue>& Sheet : *OutSheets)
			{
				...
				Sheet->AsObject()->TryGetObjectField(TEXT("properties"), SheetProperties);

				if (SheetProperties)
				{
					int32 LoopSheetId;
					(*SheetProperties)->TryGetNumberField(TEXT("sheetId"), LoopSheetId);

					if (LoopSheetId == InSheetId)
					{
						const TSharedPtr<FJsonObject>* GridProperties;
						(*SheetProperties)->TryGetObjectField(TEXT("gridProperties"), GridProperties);

						if (GridProperties)
						{
							(*GridProperties)->TryGetNumberField(TEXT("rowCount"), RowCount);
							(*GridProperties)->TryGetNumberField(TEXT("columnCount"), ColumnCount);
						}
...
{	
	...
	{		
		...
		if (FJsonSerializer::Deserialize(JsonReader, PostResponse) && PostResponse.IsValid())
		{
			...
			const FString AccessToken = PostResponse->GetStringField(TEXT("access_token"));
	...
	FHttpRequestPtr Request, FHttpResponsePtr Response, FRuntimeDataTableCallbackInfo& CallbackInfo)
{
	...
	CallbackInfo.ResponseAsString = CallbackInfo.bWasSuccessful && Response.IsValid() ? Response->GetContentAsString() : "503 Service unavailable";
	CallbackInfo.ResponseCode = Response.IsValid() ? Response->GetResponseCode() : 503;
