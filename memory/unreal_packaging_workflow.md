...
**2026-07-05 UPDATE — long-path gotcha PERMANENTLY FIXED so the Editor's own Package button works.** User rightly insisted packaging should work from the Editor menu (File→Package Project), not only via the command-line junction. Editor-menu Package always uses the REAL long project path (`-project="D:/mirmironnnn oxirgiiii/..."`), never the `D:\O` junction — so it kept hitting `LogCook: Error: Couldn't save package, filename is too long (273 >= 260)` on the 4 assets in `Content/Statik_tree/Tuvak_gullar/Collection_Indoor_outdoor_plant_163_concrete_dirt_vase_pot_palm_cactus/` (01,04,25,28) → ExitCode=1 → UAT "Cook failed" (ExitCode=25). ROOT FIX: set Windows registry `HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem\LongPathsEnabled = 1` (was 0). UE5 honors it — `FWindowsPlatformMisc::GetMaxPathLength()` returns 32767 instead of 260, so the cook 260 check passes. Set via elevated PowerShell `Start-Process powershell -Verb RunAs` (user is NOT admin in the CC process; UAC prompt approved by user). REQUIRES restarting the Editor (running process cached old 260 at startup). After restart, Editor-menu Package works directly — no junction, no rename, no command line needed. The junction `D:\O` still exists as a fallback but is no longer required.

**TWO blocking gotchas, both must be handled (pre-2026-07-05; long-path now solved via registry above):**
