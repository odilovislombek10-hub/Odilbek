...
**2. Scalability level contradicted itself across 3 ini files — AND was missing 5 of 11 categories entirely (root cause of "Lumen/RT settings don't take effect").**
`Config/DefaultEngine.ini`'s `[/Script/Engine.GameUserSettings]` section had `OverallScalabilityLevel=3` / `sg.ShadowQuality=3` etc (High) while `Config/DefaultDeviceProfiles.ini` and `Config/DefaultScalability.ini` forced the same groups to `=4` (Cinematic) via `[ScalabilityGroups]`/device profile CVars (which have `SetByScalability` priority, lower than `SetByProjectSetting` — confirmed via log: `Setting the console variable '...' with 'SetByScalability' was ignored`). First pass fixed `DefaultEngine.ini`'s 6 listed categories to `=4`, but **that section was only ever listing 6 of the 11 `sg.*` scalability groups** (`ViewDistanceQuality`, `ShadowQuality`, `PostProcessQuality`, `TextureQuality`, `EffectsQuality`, `ShadingQuality`) — it never had `sg.ResolutionQuality`, `sg.AntiAliasingQuality`, `sg.ReflectionQuality`, `sg.FoliageQuality`, or **`sg.GlobalIlluminationQuality`** at all. Because `UGameUserSettings` (`SetByGameSetting`, high priority) applies its own C++ default (3/Epic) for any `sg.*` category not explicitly present in its ini section — overriding the correct `=4` that `DefaultScalability.ini` set moments earlier at boot — this **silently capped Lumen GI, Lumen/Reflections, Foliage, and AA quality at Epic (3) instead of Cinematic (4)**, live-verified via `SearchCVars` (`sg.GlobalIlluminationQuality`, `sg.ReflectionQuality`, `sg.FoliageQuality`, `sg.AntiAliasingQuality` all read `3`). This is what the user meant by "Lumen + raytracing settings aren't working, something is limiting them" — the ini *said* Cinematic everywhere else, but this one incomplete section silently downgraded the categories that matter most for RT/Lumen quality. Also found `sg.ResolutionQuality` reading `0` (should default `100`) for the same reason — harmless in practice only because `r.ScreenPercentage=100` is set directly elsewhere with higher priority and wins.
**Fix (final):** `DefaultEngine.ini`'s `[/Script/Engine.GameUserSettings]` now explicitly lists all 11 groups at `=4` (`sg.ResolutionQuality=100`, the rest `=4`), matching `DefaultDeviceProfiles.ini`/`DefaultScalability.ini` exactly. **How to apply:** if a *new* scalability-driven quality category is ever reported as not matching the rest, check whether it's simply missing from this ini section — `UGameUserSettings` treats "absent" as "reset to engine default," not "inherit from Scalability.ini," which is the trap here.
